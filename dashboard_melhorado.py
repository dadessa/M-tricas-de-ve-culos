# -*- coding: utf-8 -*-
"""
Métricas de Veículos (v4.3-fix2)
- Fonte de dados: Google Sheets (CSV público) ou arquivo local (EXCEL_PATH)
- Persistência: VALORES_JSON_PATH (JSON) para Valor Planejado/Pago (permanece após atualizar página ou redeploy)
- Filtros populados na 1ª carga; fallback se DF_BASE vier vazio
- Gráficos:
  * Status
  * Top 10 Cidades
  * Total de Visualizações (Julho/Agosto/Setembro)
  * Top 10 Sites por Média Trimestral (Jul/Ago/Set)
- Tabela "Dados detalhados": contínua (sem paginação)
- Filtro: "Somente Valor Pago ≠ 0"
- Export: Excel/CSV + PDF (gráficos + tabela)
"""
...
# ========= CONFIG =========
EXCEL_PATH = os.getenv("EXCEL_PATH", "Recadastramento (respostas).xlsx")
...
def _resolve_views(df: pd.DataFrame, mes: str) -> Optional[str]:
    ...
    aliases = [f"visualizacoes_{mes}", f"vizualizacoes_{mes}", f"total_de_visualizacoes_{mes}", f"total_de_vizualizacoes_{mes}"]
...
# ========= PERSISTÊNCIA (VALORES_JSON_PATH) =========
def _safe_dir_for(path: str):
    d = os.path.dirname(os.path.abspath(path))
    if d and not os.path.exists(d):
        os.makedirs(d, exist_ok=True)

def _init_valores_file():
    try:
        _safe_dir_for(VALORES_JSON_PATH)
        if not os.path.exists(VALORES_JSON_PATH):
            with open(VALORES_JSON_PATH, "w", encoding="utf-8") as f:
                f.write("{}")
    except Exception as e:
        print("[init] Falha garantindo valores_financeiros.json:", e)
...
def _read_persisted_vals() -> dict:
    try:
        if not os.path.exists(VALORES_JSON_PATH):
            return {}
        with open(VALORES_JSON_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
...
def _merge_persisted_into_df(df: pd.DataFrame, persisted: dict) -> pd.DataFrame:
    df = df.copy()
    def _get(nome, key):
        try: return float(persisted.get(str(nome), {}).get(key, 0) or 0)
...
def _read_csv_robusto(path_or_url: str) -> pd.DataFrame:
    return pd.read_csv(path_or_url, sep=None, engine="python", encoding="utf-8-sig", dtype=str, na_filter=False)
...
def _prepare_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "status" in df.columns: df["status"] = df["status"].astype(str).str.upper()

    # Motivo
    mot_alias = ["motivo","motivo_da_reprovacao","motivo_de_reprovacao","motivo_reprovacao","motivo_reprova","motivo_reprov"]
    mot = _find_first(df, mot_alias) or _find_by_tokens(df, ["motivo","reprov"])
    if mot and mot != "motivo": df["motivo"] = df[mot].astype(str)
    elif "motivo" not in df.columns: df["motivo"] = ""
    df["motivo"] = df["motivo"].fillna("").astype(str).str.strip()

    # Views por mês (Jul/Ago/Set)
    ...
    df["total_visualizacoes"] = df.get("visualizacoes_julho",0.0) + df.get("visualizacoes_agosto",0.0) + df.get("visualizacoes_setembro",0.0)
    df["media_trimestral"] = df[["visualizacoes_julho","visualizacoes_agosto","visualizacoes_setembro"]].mean(axis=1)

    return df

def load_data() -> pd.DataFrame:
    # 1) Sheets (CSV público)
    if SHEETS_CSV_URL:
        try:
    ...
    try:
        ...
        cols = ["nome_fantasia","nome_do_veiculo","cidade","status","motivo","categoria",
...
# Base inicial (pode vir vazio; teremos fallback no callback)
DF_BASE = load_data()
...
def style_fig(fig, theme="light"):
    ...
    fig.update_layout(template=c["template"], colorway=c["colorway"], paper_bgcolor=c["paper"], plot_bgcolor=c["plot"],
                      font=dict(color=c["font"], size=13), title=dict(font=dict(color=c["font"], size=16)),
                      legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(color=c["font"])),
                      margin=dict(l=12, r=12, t=48, b=12), uniformtext_minsize=12, uniformtext_mode="hide",
                      hoverlabel=dict(bgcolor="rgba(15,23,42,0.95)" if theme=="dark" else "#ffffff",
                                      font_color=c["font"], bordercolor="rgba(0,0,0,0)"))
    fig.update_xaxes(gridcolor=c["grid"], zerolinecolor=c["grid"], tickfont=dict(color=c["font"]), title_font=dict(color=c["font"]))
    fig.update_yaxes(gridcolor=c["grid"], zerolinecolor=c["grid"], tickfont=dict(color=c["font"]), title_font=dict(color=c["font"]))
    fig.update_traces(selector=dict(type="bar"), textfont_color=c["font"], marker_line_width=0, opacity=0.95 if theme=="dark" else 1.0)
    return fig
...
def kpi_card(kpi_id: str, label: str):
    return html.Div(className="card kpi", children=[html.P(label, className="kpi-title"), html.H2(id=kpi_id, className="kpi-value")])
...
app.layout = html.Div(className="light", id="root", children=[
    ...
    html.Div(className="container", children=[
        ...
        html.Div(className="navbar", children=[
            html.Div(className="brand", children=[html.Div("📊", style={"fontSize":"20px"}), html.H1("Métricas de Veículos"), html.Span("v4.3-fix2", className="badge")]),
            html.Div(className="actions", children=[
                dcc.RadioItems(id="theme-toggle", options=[{"label":"Claro","value":"light"},{"label":"Escuro","value":"dark"}],
                               value="light", inline=True, inputStyle={"marginRight":"6px","marginLeft":"10px"}, style={"marginRight":"8px"}),
                ...
                dcc.Download(id="download_excel"), dcc.Download(id="download_pdf"),
            ]),
        ]),
        # Filtros
        html.Div(className="panel", children=[
            html.Div(className="filters", children=[
                html.Div(children=[html.Div("Cidade", className="label"), dcc.Dropdown(id="f_cidade", options=[], multi=True, placeholder="Selecione cidades…")]),
                html.Div(children=[html.Div("Status", className="label"), dcc.Dropdown(id="f_status", options=[], multi=True, placeholder="Selecione status…")]),
                html.Div(children=[html.Div("Categoria", className="label"), dcc.Dropdown(id="f_categoria", options=[], multi=True, placeholder="Selecione categorias…")]),
                html.Div(children=[html.Div("Nome do Veículo (selecione um ou vários)", className="label"),
                                   dcc.Dropdown(id="f_sites", options=[], multi=True, placeholder="Selecione sites…", clearable=True, searchable=True)]),
                html.Div(children=[html.Div("Filtro de Valor Pago", className="label"),
                                   dcc.Checklist(id="f_valor_nzero", options=[{"label":"Somente Valor Pago ≠ 0","value":"nz"}], value=[], inline=True,
                                                 inputStyle={"marginRight":"6px","marginLeft":"10px"})]),
                html.Div(children=[html.Div("Ordenação dos gráficos", className="label"),
                                   dcc.RadioItems(id="sort-order", options=[{"label":"Decrescente","value":"desc"},{"label":"Crescente","value":"asc"}],
                                                  value="desc", inline=True, inputStyle={"marginRight":"6px","marginLeft":"10px"})]),
            ]),
        ]),
        # KPIs
        html.Div(className="kpis", children=[kpi_card("kpi_total","Total de Veículos"), kpi_card("kpi_aprov","Aprovados"),
                                             kpi_card("kpi_reprov","Reprovados"), kpi_card("kpi_cidades","Cidades")]),
        # Gráficos
        html.Div(className="grid-2", children=[html.Div(className="card", children=[dcc.Graph(id="g_status", config={"displayModeBar":False})]),
                                               html.Div(className="card", children=[dcc.Graph(id="g_top_cidades", config={"displayModeBar":False})])]),
        html.Div(className="grid-2", children=[html.Div(className="card", children=[dcc.Graph(id="g_meses", config={"displayModeBar":False})]),
        ...
        html.Div(className="panel", children=[
            ...
            html.Div(className="card", children=[
                dash_table.DataTable(
                    id="tbl", page_action="none", sort_action="native", filter_action="native", fixed_rows={"headers":True},
...
def init_filter_options(_theme):
    d = load_data()
...
def atualizar(f_cidade, f_status, f_categoria, f_sites, f_valor_nzero, order, n_reload, theme, store_vals):
    # Base resiliente (se DF_BASE estiver vazio, força nova carga)
    if (n_reload and n_reload > 0):
        base = load_data()
    else:
        try:
            base_ok = (DF_BASE is not None) and (len(DF_BASE) > 0)
        except Exception:
            base_ok = False
        base = DF_BASE if base_ok else load_data()

    # Merge persistência (arquivo + store de sessão)
    ...
    if "status" in dff and not dff.empty:
        ...
        g1.columns = ["status","qtd"]; g1 = g1.sort_values("qtd", ascending=ascending)
    ...
    fig_meses = px.bar(g3, x="Mês", y="Visualizações", text="Visualizações", title="Total de Visualizações por Mês (Jul/Ago/Set)", color="Mês", color_discrete_sequence=seq3)
    ...
    # Top Sites por Média Trimestral
    if {"nome_fantasia","media_trimestral"}.issubset(dff.columns) and not dff.empty:
        ...
        fig_sites.update_traces(texttemplate="%{text:.0f}", hovertemplate="%{y}<br>Média trimestral: %{x:.0f}<extra></extra>")
    ...
    cols_order = ["nome_do_veiculo","cidade","status","motivo","media_trimestral","valor_planejado","valor_pago","saldo"]
    friendly = {"nome_do_veiculo":"Nome do Veículo","cidade":"Cidade","status":"Status","motivo":"Motivo",
    ...
    for c in present:
        ...
        if c == "media_trimestral": col_def.update({"type":"numeric","format":fmt_int0})
        if c in ["valor_planejado","valor_pago","saldo"]: col_def.update({"type":"numeric","format":fmt_money})
        if c in ["valor_planejado","valor_pago"]: col_def.update({"editable":True})
        columns.append(col_def)
    data = dff[present].to_dict("records")

    return (f"{total}", f"{aprov}", f"{reprov}", f"{cidades_qtd}", fig_status, fig_cidades, fig_meses, fig_sites, data, columns)

# ========= Persistir edições (tabela → JSON + Store) =========
@app.callback(
...
def persistir_valores(_, table_data, store_vals):
    ...
    if table_data:
        for row in table_data:
            ...
            try: entry["valor_planejado"] = float(row.get("valor_planejado", entry.get("valor_planejado", 0)) or 0)
            except Exception: entry["valor_planejado"] = float(entry.get("valor_planejado", 0) or 0)
            try: entry["valor_pago"] = float(row.get("valor_pago", entry.get("valor_pago", 0)) or 0)
            except Exception: entry["valor_pago"] = float(entry.get("valor_pago", 0) or 0)
            merged[nome] = entry
        _write_persisted_vals(merged)
    return merged

# ========= Atualizar filtros via botão "Atualizar dados" =========
@app.callback(
    Output("f_cidade","options"),
    Output("f_status","options"),
    Output("f_categoria","options"),
...
def _filtered_df_for_export(f_cidade, f_status, f_categoria, f_sites, nz_flag) -> pd.DataFrame:
    base = load_data()  # export sempre com dados mais recentes
    persisted = _read_persisted_vals()
    dff = _filtrar(base, f_cidade, f_status, f_categoria, f_sites)
    dff = _merge_persisted_into_df(dff, persisted)
    if nz_flag: dff = dff[(dff["valor_pago"].abs() > 0)]
    cols_export = ["nome_do_veiculo","cidade","status","motivo","media_trimestral","valor_planejado","valor_pago","saldo"]
...
def exportar_excel(n, f_cidade, f_status, f_categoria, f_sites, f_valor_nzero):
    df = _filtered_df_for_export(f_cidade, f_status, f_categoria, f_sites, nz_flag=("nz" in (f_valor_nzero or [])))
    try:
...
def exportar_pdf(n, f_cidade, f_status, f_categoria, f_sites, f_valor_nzero, order, theme):
    dff = _filtered_df_for_export(f_cidade, f_status, f_categoria, f_sites, nz_flag=("nz" in (f_valor_nzero or [])))
    ascending = (order == "asc")
    pdf_theme = "light"
    ...
    def _to_pdf(bytes_io):
        ...
        doc = SimpleDocTemplate(bytes_io, pagesize=page_size, leftMargin=left_margin, rightMargin=right_margin, topMargin=top_margin, bottomMargin=bottom_margin)
        ...
        for i, fig in enumerate(figs):
            ...
            if (i % 2 == 1) or (i == len(figs)-1):
                t = Table([[row_imgs[0]] + ([row_imgs[1]] if len(row_imgs) > 1 else [])], colWidths=[col_w, col_w] if len(row_imgs) > 1 else [col_w])
                t.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"TOP"),("LEFTPADDING",(0,0),(-1,-1),0),("RIGHTPADDING",(0,0),(-1,-1),0),
                                       ("TOPPADDING",(0,0),(-1,-1),0),("BOTTOMPADDING",(0,0),(-1,-1),0)]))
                story += [t, Spacer(1,8)]
                row_imgs = []

        labels = {"nome_do_veiculo":"Nome do Veículo","cidade":"Cidade","status":"Status","motivo":"Motivo",
                  "media_trimestral":"Média Trimestral","valor_planejado":"Valor Planejado","valor_pago":"Valor Pago","saldo":"Saldo"}
        col_keys = [c for c in ["nome_do_veiculo","cidade","status","motivo","media_trimestral","valor_planejado","valor_pago","saldo"] if c in dff.columns]
        ...
        for _, row in dff[col_keys].iterrows():
            ...
            for k in col_keys:
                v = row[k]
                if k == "media_trimestral": line.append(Paragraph(fmt_int(v), cell_text))
                elif k in ["valor_planejado","valor_pago","saldo"]: line.append(Paragraph(fmt_money(v), cell_text))
                elif k in ["nome_do_veiculo","motivo"]: line.append(Paragraph(str(v), cell_wrap))
                else: line.append(Paragraph(str(v), cell_text))
            data.append(line)

        weights = {"nome_do_veiculo":3.2,"cidade":1.2,"status":1.1,"motivo":3.2,"media_trimestral":1.4,"valor_planejado":1.5,"valor_pago":1.5,"saldo":1.5}
        ...
        styles_tbl = [("BACKGROUND",(0,0),(-1,0),colors.HexColor("#111827")),("TEXTCOLOR",(0,0),(-1,0),colors.white),
