# -*- coding: utf-8 -*-
"""
Métricas de Veículos (v4.3-final)
- Sempre recarrega os dados da fonte
- Badge de status da carga
- Persistência de valores editáveis
- Gráficos, filtros e tabela ajustados
"""
...
def _resolve_views(df: pd.DataFrame, mes: str) -> Optional[str]:
    ...
    aliases = [
        f"visualizacoes_{mes}", f"vizualizacoes_{mes}",
...
def _init_valores_file():
    try:
        ...
        print("[init] erro ao criar json persistente:", e)
...
def _read_persisted_vals() -> dict:
    try:
        ...
        print("[persist] erro leitura JSON:", e)
        return {}
...
def _write_persisted_vals(data: dict) -> None:
    try:
        ...
        print("[persist] erro escrita JSON:", e)
...
def _read_excel_robusto(path: str) -> pd.DataFrame:
    try:
        ...
        print("[load_data] falha openpyxl:", e1)
        try:
            return pd.read_excel(path, dtype=str)
        except Exception as e2:
            print("[load_data] falha excel genérico:", e2)
            try:
                return _read_csv_robusto(path)
            except Exception as e3:
                print("[load_data] falha ler como CSV local:", e3)
                raise
...
def _prepare_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # nome fantasia / veículo
    if "nome_fantasia" not in df.columns:
        cand = _find_first(df, ["nome_do_veiculo", "nome", "nome_site", "site"])
        if not cand:
    ...
    if "url" not in df.columns:
        cand = _find_first(df, ["url", "link", "url_do_site"])
    ...
    # motivo
    mot_alias = ["motivo", "motivo_de_reprovacao", "motivo_reprovacao", "motivo_reprova"]
    ...
    # visualizações por mês (julho/agosto/setembro)
    for mes in ["julho", "agosto", "setembro"]:
        col = _resolve_views(df, mes)
        if col:
            df[f"visualizacoes_{mes}"] = clean_numeric(df[col])
        else:
            print(f"[prepare] não achou views de {mes.upper()}, colunas originais:", original)
...
def load_data() -> pd.DataFrame:
    ...
    if SHEETS_CSV_URL:
        try:
            ...
            print("[load_data] sheets carregado OK, linhas:", len(raw))
            return _prepare_df(raw)
        except Exception as e:
            print("[load_data] falhou Sheets:", e)
    ...
    try:
        ...
        print("[load_data] falha local:", e)
        # retorna DataFrame vazio com colunas mínimas para não quebrar
        cols = [
            "nome_do_veiculo", "cidade", "status", "motivo",
            "categoria", "visualizacoes_julho", "visualizacoes_agosto",
            "visualizacoes_setembro", "total_visualizacoes", "media_trimestral", "url"
        ]
        empty = pd.DataFrame(columns=cols)
        return _prepare_df(empty)

# não usar DF_BASE fixo — vamos sempre recarregar
# DF_BASE = load_data()
    ...
    html.Div(className="container", children=[
        html.Div(className="navbar", children=[
            html.Div(className="brand", children=[
                ...
                html.Span("v4.3-final", className="badge")
            ]),
            html.Div(className="actions", children=[
                ...
                html.Span(id="data-status", className="badge", style={"marginLeft": "8px"}),
            ]),
        ]),
...
# ========== CALLBACK: popula filtros na 1ª carga =========
@app.callback(
...
def _filtrar(base: pd.DataFrame, cidade, status, categoria, sites) -> pd.DataFrame:
    ...
    if sites:
        dff = dff[dff["nome_do_veiculo"].astype(str).isin(sites)]
    return dff
...
def atualizar(f_cidade, f_status, f_categoria, f_sites, f_valor_nzero, order, n_reload, theme, store_vals):
    ...
    if "status" in dff and not dff.empty:
        ...
        fig_status = px.bar(
            g1, x="status", y="qtd", text="qtd", title="Distribuição por Status", color="status",
            color_discrete_map={"APROVADO": "#22C55E", "REPROVADO": "#EF4444", "APROVADO PARCIAL": "#F59E0B",
                                "PENDENTE": "#A78BFA", "INSTA": "#06B6D4"},
        )
    ...
    if "cidade" in dff and not dff.empty:
        ...
        fig_cidades = px.bar(
            base_cid, x="cidade", y="qtd", text="qtd", title="Top 10 Cidades", color="cidade", color_discrete_sequence=seq
        )
    ...
    fig_meses = px.bar(
        g3, x="Mês", y="Visualizações", text="Visualizações", title="Visualizações por Mês (Jul/Ago/Set)",
    ...
    if {"nome_fantasia", "media_trimestral"}.issubset(dff.columns) and not dff.empty:
        ...
        fig_sites = px.bar(
            g4, x="media_trimestral", y="nome_fantasia", orientation="h", text="media_trimestral",
...
# ========== CALLBACK: persistir edições tabela ==========
@app.callback(
...
# ========== CALLBACK: recarregar filtros pelo botão ==========
@app.callback(
    Output("f_cidade", "options"),
    Output("f_status", "options"),
    Output("f_categoria", "options"),
...
# ========== CALLBACK: badge de status da carga ==========
@app.callback(
...
def show_data_status(_n, _theme):
    try:
        df = load_data()
        src = "Google Sheets (CSV)" if SHEETS_CSV_URL else "Arquivo local"
        return f"{src}: {len(df)} linhas"
    except Exception as e:
        hint = ""
        if SHEETS_CSV_URL and "export?format=csv" not in SHEETS_CSV_URL:
            hint = " — verifique URL de export CSV"
        return f"Erro ao carregar dados: {type(e).__name__}{hint}"
...
def exportar_excel(n, f_cidade, f_status, f_categoria, f_sites, f_valor_nzero):
    ...
    try:
        ...
        print("[export_excel] falhou to_excel:", e)
...
def exportar_pdf(n, f_cidade, f_status, f_categoria, f_sites, f_valor_nzero, order, theme):
    ...
    if {"nome_do_veiculo", "media_trimestral"}.issubset(dff.columns) and not dff.empty:
        ...
        fig_sites = px.bar(
            g4, x="media_trimestral", y="nome_do_veiculo", orientation="h", text="media_trimestral",
            title="Top 10 Sites (Média Trimestral Jul/Ago/Set)",
    ...
    def _to_pdf(bytes_io):
        ...
        m = 12 * mm
        left_margin = right_margin = top_margin = bottom_margin = m
        ...
        def fig_to_rlimage(fig, width_pt):
            try:
                ...
                print("[export_pdf] falha imagem:", e)
                return None
        ...
        for i, fig in enumerate(figs):
            ...
            row_imgs.append(rlimg or Paragraph("**Gráfico indisponível**", cell_text))
