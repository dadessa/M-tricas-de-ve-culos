...
- Corrige SyntaxError: unmatched ')'
- Evita quebra de linha dentro de strings e fecha todos os parênteses/colchetes/chaves
- Fallback se DF_BASE vier vazio (carrega novamente)
- Filtros populados na 1ª carga (sem depender de DF_BASE)
- Persistência de valores editáveis (JSON)
"""
...
def _prepare_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for mes in ["julho","agosto","setembro"]:
        ...
        if col: df[f"visualizacoes_{mes}"] = clean_numeric(df[col])
        else:
...
def _filtered_df_for_export(f_cidade, f_status, f_categoria, f_sites, nz_flag) -> pd.DataFrame:
    base = load_data()  # export sempre atualizado
...
def exportar_pdf(n, f_cidade, f_status, f_categoria, f_sites, f_valor_nzero, order, theme):
    ...
    if "status" in dff and not dff.empty:
        ...
        fig_status = px.bar(g1, x="status", y="qtd", text="qtd", title="Distribuição por Status", color="status",
                            color_discrete_map={"APROVADO":"#22C55E","REPROVADO":"#EF4444","APROVADO PARCIAL":"#F59E0B","PENDENTE":"#A78BFA","INSTA":"#06B6D4"})
        fig_status.update_traces(textposition="outside"); fig_status.update_layout(showlegend=True, xaxis=dict(categoryorder="array", categoryarray=g1["status"].tolist()))
    else:
        fig_status = px.bar(title="Distribuição por Status")
    ...
    if "cidade" in dff and not dff.empty:
        base_cid = dff["cidade"].value_counts().reset_index()
        base_cid.columns = ["cidade","qtd"]; base_cid = base_cid.sort_values("qtd", ascending=False).head(10); base_cid = base_cid.sort_values("qtd", ascending=ascending)
        seq = get_sequence(pdf_theme, len(base_cid))
        fig_cidades = px.bar(base_cid, x="cidade", y="qtd", text="qtd", title="Top 10 Cidades", color="cidade", color_discrete_sequence=seq)
        fig_cidades.update_traces(textposition="outside"); fig_cidades.update_layout(showlegend=False, xaxis=dict(categoryorder="array", categoryarray=base_cid["cidade"].tolist()))
    else:
        fig_cidades = px.bar(title="Top 10 Cidades")
    ...
    fig_meses = px.bar(g3, x="Mês", y="Visualizações", text="Visualizações", title="Total de Visualizações por Mês (Jul/Ago/Set)", color="Mês", color_discrete_sequence=seq3)
    fig_meses.update_traces(texttemplate="%{text:.0f}", textposition="outside"); fig_meses.update_layout(showlegend=False, xaxis=dict(categoryorder="array", categoryarray=g3["Mês"].tolist()))
    ...
    if {"nome_do_veiculo","media_trimestral"}.issubset(dff.columns) and not dff.empty:
        g4 = dff.nlargest(10, "media_trimestral")[["nome_do_veiculo","media_trimestral"]]; g4 = g4.sort_values("media_trimestral", ascending=ascending)
